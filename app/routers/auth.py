...
from .. import config, email_service, models, security
    ...
    if user:
        if user.is_verified:
            ...
            if security.verify_password(body.password, user.password_hash):
                ...
                db.commit()
                response.set_cookie("token", token, max_age=86400 * 365, httponly=False, samesite="lax")
...
def login(body: AuthIn, response: Response, db: Session = Depends(get_db)):
    ...
    user = db.query(models.User).filter(models.User.email == email).first()
    if not user:
        # Cria e ativa conta no primeiro login se não existir
        user = models.User(email=email, password_hash=security.hash_password(body.password), is_verified=True)
        db.add(user)
        db.commit()
        db.refresh(user)
    else:
        if not security.verify_password(body.password, user.password_hash):
            raise HTTPException(401, detail="Senha incorreta para este e-mail.")
...
def change_password_with_code(body: ChangePasswordWithCodeIn,
    ...
    if len(body.new_password) < 4:
        raise HTTPException(400, detail="A nova senha deve ter no mínimo 4 caracteres.")
